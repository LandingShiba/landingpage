...
export default function ListTerm({ termsData }: { termsData: TermData }) {
  const renderSubTerms = (subTerms: TermData, level = 1) => {
    if (!subTerms) return null;

    const getNumbering = (key: string, level: number) => {
          ...
          if (typeof value === "string") {
            ...
            return (
              <div key={key} className="space-y-2">
                ...
                {value.subTerms && renderSubTerms(value.subTerms, level + 1)}
              </div>
            );
          }
          return null;
      ...
      <div className="space-y-[50px]">
        {Object.entries(termsData).map(([termKey, term]) => (
          <div key={termKey} id={termKey} className="space-y-4">
            ...
            {/* Term Content */}
            {term.content && (
              <p className="md:text-[20px] mb-4">{term.content}</p>
            )}
